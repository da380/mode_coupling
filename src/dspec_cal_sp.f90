...
!  real(sp), parameter :: tol = 1.e-5_sp
  ...
  complex(spc), dimension(:), allocatable :: vs
  complex(spc), dimension(:,:), allocatable :: vr
  ...
  do iw = 1,nt
     ...
     if(ntb /= 0) then
        ...
        call cgetrf(tbd,tbd,atb,nelem,ipivot,info)
        ...
        call cgetrs('N',tbd,1,atb,nelem,ipivot,vsd,nelem,info)
        do ib = 1,ntb
...
!     end if          
     ...
     do it = 1,maxit
        ...
        if(ntb /= 0) then
           ...
           call cgetrs('T',tbd,1,atb,nelem,ipivot,vsd,nelem,info)
           do ib = 1,ntb
              vt(tbl(ib,2)+1:tbl(ib,2)+tbl(ib,4)) = & 
                   vsd(tbl(ib,3)+1:tbl(ib,3)+tbl(ib,4),1)
           end do
