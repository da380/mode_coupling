...
c     if the n of the read file does not equal nord, stop
      if(n.ne.nord) pause 'error 1 in fetch'
c     if the mode types do not agree, stop
      if(iq.eq.it.and.jcom.ne.2) pause 'error 2 in fetch'
c     if a radial mode, and jcom /= 1, stop
      if(iq.eq.is.and.l.eq.0.and.jcom.ne.1) pause 'error 3 in fetch'
c     if a spheroidal mode, and jcom /= 3, stop
